...
const Dashboard: React.FC = () => {
  ...
  const fetchProfile = async (userId: string) => {
    try {
      ...
      let currentActions = DEFAULT_QUICK_ACTIONS;
      let currentSocial = DEFAULT_SOCIAL_LINKS;

      if (data && data.content) {
        currentProfile = data.content.profile || DEFAULT_PROFILE;
        if (data.content.actions) currentActions = data.content.actions;
        if (data.content.links) currentSocial = data.content.links;
      }

      // Verifica se há status de aprovado na URL
      const params = new URLSearchParams(window.location.search);
      const isApproved = params.get('status') === 'approved';

      if (isApproved && !currentProfile.isPremium) {
          // SE O PAGAMENTO FOI APROVADO E O USUÁRIO AINDA NÃO ERA PREMIUM NO BANCO:
          // Salva automaticamente o status premium
          ...
          const { error: updateError } = await supabase.from('profiles').upsert({
              ...
              updated_at: new Date(),
              content: { profile: updatedProfile, actions: currentActions, links: currentSocial }
          });
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-black text-gray-900 dark:text-white relative">
      ...
      {paymentSuccess && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 pointer-events-none">
             <div className="bg-green-600 text-white px-6 py-4 rounded-xl shadow-2xl animate-slide-up flex items-center gap-4 border border-green-400 pointer-events-auto">
                <i className="fa-solid fa-circle-check text-2xl"></i>
                <div>
                    <h3 className="font-bold">Pagamento Confirmado!</h3>
                    <p className="text-sm">Seu plano Premium vitalício está ativo.</p>
                </div>
                <button onClick={() => setPaymentSuccess(false)} className="ml-2 hover:text-green-200"><i className="fa-solid fa-times"></i></button>
             </div>
          </div>
      )}
      ...
      {showQRCodeModal && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
             ...
             <div className="relative bg-white dark:bg-zinc-900 border border-gold/20 p-8 rounded-2xl shadow-2xl max-w-sm w-full text-center animate-slide-up">
                 ...
                 <button onClick={handleDownloadQR} className="w-full bg-gold hover:bg-yellow-400 text-black font-bold py-3 rounded-lg flex items-center justify-center gap-2">
      ...
      <header className="fixed top-0 w-full bg-white dark:bg-zinc-900 border-b border-gray-200 dark:border-zinc-800 z-50 h-16 flex items-center justify-between px-4 lg:px-8">
        <div className="flex items-center gap-3">
             ...
             <button onClick={() => status.clickable && setShowPremiumModal(true)} className={`flex items-center gap-2 px-3 py-1 rounded-full text-xs font-bold ml-2 ${status.color} ${status.clickable ? 'hover:scale-105 cursor-pointer shadow-lg' : ''} transition-all`}>
         ...
         <section className="bg-white dark:bg-zinc-900 p-6 rounded-xl border border-gray-200 dark:border-zinc-800 shadow-sm">
            ...
            <div className="flex flex-col sm:flex-row gap-2">
                <div className="flex-1 flex items-center bg-gray-100 dark:bg-black rounded-lg border border-gray-300 dark:border-zinc-700 overflow-hidden">
                    <span className="pl-3 pr-1 text-gray-500 text-sm">{window.location.host}/</span>
                    <input type="text" value={profileData.alias} onChange={handleAliasChange} className="flex-1 bg-transparent p-3 outline-none font-bold text-indigo-500" placeholder="seu-nome" />
                    <button onClick={handleCopyLink} title="Copiar link" className="px-3 py-3 text-gray-400 hover:text-gold transition-colors border-l border-zinc-800 hover:bg-zinc-900">
                        {copiedLink ? <i className="fa-solid fa-check text-green-500"></i> : <i className="fa-regular fa-copy"></i>}
                    </button>
                    <button onClick={handleQRClick} title="Gerar QR Code" className={`px-3 py-3 transition-colors border-l border-zinc-800 hover:bg-zinc-900 ${profileData.isPremium ? 'text-gray-400 hover:text-gold' : 'text-gray-600 hover:text-red-400'}`}>
                         {profileData.isPremium ? <i className="fa-solid fa-qrcode"></i> : <i className="fa-solid fa-lock text-xs"></i>}
                    </button>
                </div>
                <button onClick={viewLiveCard} className="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-3 rounded-lg font-bold transition-colors">Acessar</button>
            </div>
         </section>
             ...
             <div className="bg-white dark:bg-zinc-900 p-6 rounded-xl border border-gray-200 dark:border-zinc-800 shadow-sm">
                 ...
                 <div className="space-y-3">
                     ...
                     <div className="flex gap-2">
                        <input type="text" placeholder="Tipo" value={profileData.document.label} onChange={(e) => handleDocumentChange('label', e.target.value)} className="w-1/3 bg-gray-50 dark:bg-black border border-gray-200 dark:border-zinc-700 p-3 rounded-lg outline-none focus:border-gold dark:text-white" />
            ...
            <div className="grid md:grid-cols-2 gap-4">
                {quickActions.map((action, idx) => (
                    <div key={idx} className="bg-gray-50 dark:bg-black p-3 rounded-lg border border-gray-200 dark:border-zinc-800">
                        ...
                        <input type="text" value={getDisplayValue(action)} onChange={(e) => handleSmartActionChange(idx, e.target.value)} className="w-full bg-transparent border-b border-zinc-700 focus:border-gold outline-none py-1 text-sm dark:text-white" />
                    </div>
                ))}
            </div>
         </section>
            ...
            <div className="space-y-3">
                {socialLinks.map((link, idx) => (
                    <div key={idx} className="flex items-center gap-3">
                        <div className="w-8 h-8 flex items-center justify-center bg-gray-100 dark:bg-black rounded border border-gray-200 dark:border-zinc-800 text-gray-500"><i className={link.icon}></i></div>
                        <input type="text" value={link.url} onChange={(e) => handleSocialChange(idx, e.target.value)} className="flex-1 bg-gray-50 dark:bg-black border border-gray-200 dark:border-zinc-700 p-2 rounded outline-none focus:border-gold text-sm dark:text-white" />
                    </div>
                ))}
            </div>
         </section>
      ...
      <div className="fixed bottom-0 w-full bg-white dark:bg-zinc-900 p-4 border-t border-gray-200 dark:border-zinc-800 z-40 flex justify-center">
        <button onClick={handleSave} disabled={isSaving} className="w-full max-w-md bg-gold hover:bg-yellow-400 text-black font-bold py-3 rounded-lg shadow-lg flex items-center justify-center gap-2 disabled:opacity-50">
