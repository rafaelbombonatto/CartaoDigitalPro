    ...
    <div className="min-h-screen bg-white dark:bg-black text-gray-900 dark:text-white pb-40 transition-colors duration-500">
      ...
      <header className="fixed top-0 w-full bg-white/90 dark:bg-zinc-900/90 backdrop-blur-xl border-b border-gray-100 dark:border-zinc-800 z-50 h-20 flex items-center justify-between px-4 sm:px-6">
        ...
        <div className="flex items-center gap-2">
             <button onClick={() => window.open(`/${profileData.alias}`, '_blank')} className="text-[10px] font-black px-3 py-1.5 rounded-lg border border-gray-200 dark:border-zinc-700 text-brand-cyan">VER CARD</button>
             <button onClick={() => supabase.auth.signOut().then(() => navigate('/'))} className="text-[10px] font-black text-red-500 px-2">SAIR</button>
        </div>
      </header>

      <div className="pt-28 px-4 max-w-xl mx-auto space-y-8">
         ...
         <section className="bg-gray-50/50 dark:bg-zinc-900/50 p-5 rounded-[2rem] border border-gray-100 dark:border-zinc-800/50 shadow-sm">
            ...
            <div className="flex items-center gap-1 bg-white dark:bg-black rounded-2xl border border-gray-200 dark:border-zinc-800 p-1 group focus-within:border-brand-cyan transition-all">
                <span className="pl-3 text-gray-400 text-xs font-bold whitespace-nowrap shrink-0">analisecardpro/</span>
                <input 
                    type="text" 
         ...
         <section className="bg-zinc-950 border border-white/5 p-6 rounded-[2.5rem] shadow-2xl overflow-hidden">
            ...
            <div className="grid grid-cols-2 gap-4">
                <div className="bg-black/40 p-5 rounded-2xl border border-white/5 flex flex-col justify-center">
                    <div className="text-[8px] text-gray-500 font-bold uppercase tracking-widest mb-1">Cliques Totais</div>
                    <div className="text-2xl font-black text-white">{totalClicks}</div>
                </div>
                <button 
                  onClick={exportCSV}
                  className="bg-zinc-900 hover:bg-zinc-800 text-white text-[9px] font-black p-5 rounded-2xl uppercase tracking-widest flex items-center justify-center gap-2 border border-white/5 transition-all text-center leading-tight"
                >
                    <i className="fa-solid fa-file-csv text-brand-cyan"></i> EXPORTAR RELATÓRIO DETALHADO
                </button>
            </div>
         </section>
         ...
         <section className="bg-white dark:bg-zinc-900 p-6 rounded-[2rem] border border-gray-100 dark:border-zinc-800 space-y-6 shadow-sm">
             ...
             <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                    ...
                    <div onClick={() => avatarInputRef.current?.click()} className="aspect-square rounded-2xl bg-gray-50 dark:bg-black border-2 border-dashed border-gray-200 dark:border-zinc-800 flex items-center justify-center cursor-pointer overflow-hidden relative group hover:border-brand-cyan transition-colors">
                ...
                <div className="space-y-2">
                    ...
                    <div onClick={() => bgInputRef.current?.click()} className="aspect-square rounded-2xl bg-gray-50 dark:bg-black border-2 border-dashed border-gray-200 dark:border-zinc-800 flex items-center justify-center cursor-pointer overflow-hidden relative group hover:border-brand-cyan transition-colors">
      ...
      <div className="fixed bottom-0 w-full bg-white/80 dark:bg-black/80 backdrop-blur-xl p-4 sm:p-6 border-t border-gray-100 dark:border-zinc-800/50 z-[60] flex justify-center pb-safe">
        <button onClick={handleSave} disabled={isSaving} className="w-full max-w-lg bg-brand-cyan hover:bg-white text-black font-black py-4 rounded-2xl shadow-2xl flex items-center justify-center gap-3 transition-all disabled:opacity-50">
