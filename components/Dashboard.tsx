  ...
  return (
    <div className="min-h-screen bg-white dark:bg-black text-gray-900 dark:text-white pb-40 transition-colors duration-500">
      ...
      <div className="pt-28 px-4 max-w-xl mx-auto space-y-10">
         ...
         <section className="bg-gray-50/50 dark:bg-zinc-900/50 p-6 rounded-[2rem] border border-gray-100 dark:border-zinc-800/50 shadow-sm transition-all hover:shadow-md">
            ...
            <div className="flex items-center gap-1 bg-white dark:bg-black rounded-2xl border border-gray-200 dark:border-zinc-800 p-1 group focus-within:border-brand-cyan transition-all shadow-sm">
                <span className="pl-3 text-gray-400 text-[11px] font-bold whitespace-nowrap shrink-0">analisecardpro.com.br/</span>
                <input 
                    type="text" 
                ...
                <button 
                  ...
                  className="p-3 text-zinc-400 hover:text-brand-cyan shrink-0 transition-colors"
                >
         ...
         <section className="bg-gray-50/50 dark:bg-zinc-900/50 p-6 rounded-[2rem] border border-gray-100 dark:border-zinc-800/50 shadow-sm space-y-6">
             ...
             <div className="space-y-4">
                 <input type="text" value={profileData.name} onChange={(e) => setProfileData({...profileData, name: e.target.value})} placeholder="Seu Nome Completo" className="w-full bg-white dark:bg-black border border-gray-200 dark:border-zinc-800 p-4 rounded-xl outline-none focus:border-brand-cyan text-xs font-bold shadow-sm transition-all" />
                 <input type="text" value={profileData.title} onChange={(e) => setProfileData({...profileData, title: e.target.value})} placeholder="Cargo ou Especialidade" className="w-full bg-white dark:bg-black border border-gray-200 dark:border-zinc-800 p-4 rounded-xl outline-none focus:border-brand-cyan text-xs font-bold shadow-sm transition-all" />
                 <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                        <label className="text-[9px] font-black text-zinc-500 uppercase tracking-widest ml-1">Tipo de Registro</label>
                        <input type="text" value={profileData.document?.label || ''} onChange={(e) => setProfileData({...profileData, document: { ...profileData.document, label: e.target.value }})} placeholder="Ex: CRECI, OAB" className="w-full bg-white dark:bg-black border border-gray-200 dark:border-zinc-800 p-4 rounded-xl outline-none focus:border-brand-cyan text-xs font-bold shadow-sm transition-all" />
                    </div>
                    <div className="space-y-1.5">
                        <label className="text-[9px] font-black text-zinc-500 uppercase tracking-widest ml-1">Número</label>
                        <input type="text" value={profileData.document?.value || ''} onChange={(e) => setProfileData({...profileData, document: { ...profileData.document, value: e.target.value }})} placeholder="000.000-0" className="w-full bg-white dark:bg-black border border-gray-200 dark:border-zinc-700 p-4 rounded-xl outline-none focus:border-brand-cyan text-xs font-bold shadow-sm transition-all" />
                    </div>
                 </div>
                 <textarea rows={3} value={profileData.bio} onChange={(e) => setProfileData({...profileData, bio: e.target.value})} placeholder="Sua bio profissional rápida..." className="w-full bg-white dark:bg-black border border-gray-200 dark:border-zinc-800 p-4 rounded-xl outline-none focus:border-brand-cyan text-xs font-medium resize-none shadow-sm transition-all" />
             </div>
         </section>
         ...
         <section className="bg-gray-50/50 dark:bg-zinc-900/50 p-6 rounded-[2rem] border border-gray-100 dark:border-zinc-800/50 shadow-sm space-y-4">
             ...
             {quickActions.map((action, idx) => (
                 <div key={idx} className="space-y-1.5 group">
                    <label className="text-[8px] font-black text-zinc-500 uppercase tracking-widest flex items-center gap-2 group-focus-within:text-brand-cyan transition-colors">
             ...
             {socialLinks.map((link, idx) => (
                 <div key={idx} className="flex gap-2">
                    ...
                    <input 
                        ...
                        className="flex-1 bg-white dark:bg-black border border-gray-200 dark:border-zinc-700 p-4 rounded-xl outline-none focus:border-brand-cyan text-[10px] font-bold shadow-sm transition-all" 
                    />
                 </div>
             ))}
         </section>
      ...
      <div className="fixed bottom-0 w-full bg-white/80 dark:bg-black/80 backdrop-blur-xl p-4 sm:p-6 border-t border-gray-100 dark:border-zinc-800/50 z-[60] flex justify-center pb-safe">
        <button onClick={handleSave} disabled={isSaving} className="w-full max-w-lg bg-brand-cyan hover:bg-white text-black font-black py-5 rounded-2xl shadow-2xl flex items-center justify-center gap-3 transition-all transform active:scale-95 disabled:opacity-50 group">
            {isSaving ? <i className="fa-solid fa-spinner fa-spin"></i> : <i className="fa-solid fa-cloud-arrow-up group-hover:animate-bounce"></i>}
